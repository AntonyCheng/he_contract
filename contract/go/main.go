...
type Seal struct {
	...
	// 印章标题
	SealTitle string `json:"seal_title"`
	// ipfs链值
	SealHash string `json:"seal_hash"`
	// 用户id
	SealUserId int `json:"user_id"`
}
...
func (ccc *ContractChainCode) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	fmt.Println("调用函数：", function, "  对象Id：", args[0])
	switch function {
	case "addContract":
...
// 将字符串数组类型数据转换为“印章”结构体
func convertToSealFromArgs(args []string) (*Seal, error) {
	if len(args) != 4 {
		return nil, errors.New("参数数量不正确. 应输入如下4个参数" +
			" (印章Id, 印章标题, Ipfs链值, 用户Id)")
	}
	sealId, err := strconv.Atoi(args[0])
	if err != nil {
		return nil, errors.New("印章Id必须是数字字符串")
	}
	sealTitle := args[1]
	sealHash := args[2]
	sealUserId, err := strconv.Atoi(args[3])
	if err != nil {
...
// 添加印章
func (ccc *ContractChainCode) addSeal(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// 将字符串数组类型数据转换为"印章"结构体
	seal, err := convertToSealFromArgs(args)
	if err != nil {
		return shim.Error(err.Error())
	}
	fmt.Println("seal:", seal)
	// 检查添加的印章ID是否已经存在
	sealIdAsString := strconv.Itoa(seal.SealId)
	sealAsBytes, err := stub.GetState(SEAL + sealIdAsString)
	if err != nil {
		return shim.Error(err.Error())
	} else if sealAsBytes != nil {
		return shim.Error("此ID印章已经存在")
	}
	// 结构体转换为json字符串
	sealAsJsonBytes, err := json.Marshal(seal)
	if err != nil {
		return shim.Error(err.Error())
	}
	// 保存到账本中
	err = stub.PutState(SEAL+sealIdAsString, sealAsJsonBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
	return shim.Success(sealAsJsonBytes)
}
...
// 更新印章
func (ccc *ContractChainCode) updateSealById(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// 将字符串数组类型数据转换为"印章"结构体
	seal, err := convertToSealFromArgs(args)
	if err != nil {
		return shim.Error(err.Error())
...
func (ccc *ContractChainCode) getUserById(stub shim.ChaincodeStubInterface, userId string) peer.Response {
	userAsBytes, err := stub.GetState(USER + userId)
	if err != nil {
		return shim.Error("查找合同失败:" + err.Error())
	} else if userAsBytes == nil {
...
// 用过印章id查询印章
func (ccc *ContractChainCode) getSealById(stub shim.ChaincodeStubInterface, sealId string) peer.Response {
	sealAsBytes, err := stub.GetState(SEAL + sealId)
	if err != nil {
		return shim.Error("查找印章失败:" + err.Error())
	} else if sealAsBytes == nil {
		return shim.Error("该id的印章不存在")
	}
	fmt.Printf("Search Response:%s\n", string(sealAsBytes))
	return shim.Success(sealAsBytes)
}
...
// 通过用户id删除用户
func (ccc *ContractChainCode) deleteUserById(stub shim.ChaincodeStubInterface, userId string) peer.Response {
	userAsBytes, err := stub.GetState(USER + userId)
	if err != nil {
		return shim.Error("查询出错:" + err.Error())
	} else if userAsBytes == nil {
		return shim.Error("找不到对应id的用户")
	}
	err = stub.DelState(USER + userId)
	if err != nil {
		return shim.Error("删除用户失败:" + USER + userId + err.Error())
	}
	return shim.Success(nil)
}

// 通过印章id删除印章
func (ccc *ContractChainCode) deleteSealById(stub shim.ChaincodeStubInterface, sealId string) peer.Response {
	sealAsBytes, err := stub.GetState(SEAL + sealId)
	if err != nil {
		return shim.Error("查询出错:" + err.Error())
	} else if sealAsBytes == nil {
		return shim.Error("找不到对应id的印章")
	}
	err = stub.DelState(SEAL + sealId)
	if err != nil {
		return shim.Error("删除用户失败:" + SEAL + sealId + err.Error())
	}
	return shim.Success(nil)
}
